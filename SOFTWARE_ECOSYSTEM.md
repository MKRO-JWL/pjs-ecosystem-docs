...
**Art. 35 — data protection impact assessment: not required.** The shop sells collectibles to consumers. It runs no profiling or automated decision-making, holds no special-category or criminal-offence data, does not monitor a publicly accessible area, and processes a small volume for a single small merchant — so none of the Art. 35(3) triggers or the German supervisory authorities' mandatory-DPIA list apply. This determination is recorded rather than assumed. Reporting on identified customers for ordinary business administration is permitted and does not disturb it; introducing profiling, scoring or behavioural targeting does, and requires the determination to be re-run — and confirmed by counsel, not by this document.
...
**Repository Analysis** — read every relevant file completely. Map dependencies. Identify patterns. Identify risks. Never assume architecture. **A backlog Issue is a pointer to a concern, never a specification.** Stubs are filed during an earlier cycle and quote code as of that day; the repo keeps moving, so re-derive the defect from the code before designing against the stub. When the two disagree, say so explicitly — to the owner and in the Issue — rather than quietly working around it. Precedent: #38 reported a defect that had been fixed 17 days before it was filed, while three *real* variants of that defect survived in the fixed code and would have been missed by implementing the stub literally.
...
- **Scope a task before opening its Issue.** A **big** Issue gets a **whole session to itself** — the full cycle at full depth, nothing else taken on beside it; it is neither batched with unrelated work nor carved into fragments that each lose the whole.
- **An Issue with both a platform and a storefront side is worked in tandem.** When a task touches the Django monolith *and* [PJsShopFront](#pjsshopfront-context), the Architecture Model covers **both** repos, the plan carries an FE and a BE workstream, and Verification is not complete until both are green. Backend-now / frontend-later ships a contract only one side honours — the failure mode `schema.yml` currency gating and the error-envelope legacy tolerance both exist to catch after the fact. The one-worker-per-repo rule still holds: a tandem cycle means one worker in each repo, not two in one.
